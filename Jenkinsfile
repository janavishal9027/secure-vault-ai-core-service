// Build pipeline for the ai-core-service.
//
...
//
// The multi-stage Dockerfile installs Python deps into a venv and stages
// them into a slim runtime image — no Python toolchain needed on the
// Jenkins agent, only Docker.
//
// ---------------------------------------------------------------------------
// Jenkins setup:
...
pipeline {
  ...
  environment {
    ...
    IMAGE_REPO  = 'kittuvittu/secure-vault-ai-core-service'
...
the target env (e.g. image-versions/ai-core-service-<env>_image.yaml):
